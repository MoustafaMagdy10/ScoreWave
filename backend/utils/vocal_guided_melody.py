...
def filter_accompaniment_patterns(
    ...
    for i, note in enumerate(notes):
        ...
        if i > 1 and i < len(notes) - 1:
            ...
            if (note['pitch'] == prev_note['pitch'] == next_note['pitch'] and
                duration < 0.15 and note['amplitude'] < 0.5):
                continue
...
def select_melodic_range(
    notes: List[Dict[str, Any]],
    vocal_range: Optional[tuple[int, int]] = None
) -> List[Dict[str, Any]]:
    """
...
def apply_vocal_guided_extraction(
    ...
    stats = {
        ...
        "reduction_pct": round((1 - len(final_notes) / original_count) * 100, 1) if original_count > 0 else 0,
